    ...
    <div class="grid gap-sm">
        ...
        <div class="bg radius-md padding-md shadow-xs col-12">
                    ...
                    <div class="file-upload">
                        <label for="importFile" class="file-upload__label btn btn--primary">
                            <span class="flex items-center">
                                ...
                                <span class="margin-left-xxs file-upload__text file-upload__text--has-max-width">Выбрать
                                    файл</span>
                            </span>
                        </label>
            ...
            <div class="margin-y-sm">
                <a href="{{ route('specs.export') }}" type="button" class="btn btn--subtle">Экспорт &#128640;</a>
            </div>
                                    ...
                                    <td class="tbl__cell text-right" role="cell">
                                        ...
                                        <div class="flex flex-wrap gap-xs">
                                            <a class="btn btn--primary btn--sm"
                                                href="{{ route('specs.edit', ['spec' => $spec->id]) }}">
                                                Изменить
                                            </a>
                                            <button class="btn btn--accent btn--sm"
                                                aria-controls="dialog-{{ $loop->index }}">Удалить</button>
                                ...
                                <div id="dialog-{{ $loop->index }}" class="dialog js-dialog" data-animation="on">
                                    <div class="dialog__content max-width-xxs" role="alertdialog"
                                        ...
                                        <div class="text-component">
                                            <h4 id="dialog-title-{{ $loop->index }}">Вы уверены, что хотите удалить
                                                продавца {{ $spec->name }}?</h4>
                                            <p id="dialog-description-{{ $loop->index }}">После подтверждения продавец
                                                будет удален <mark>безвозвратно</mark>!!!.</p>
                                        </div>
                                        ...
                                        <footer class="margin-top-md">
                                            <div class="flex justify-end gap-xs flex-wrap">
                                                ...
                                                <form method="POST"
                                                    action="{{ route('specs.destroy', ['spec' => $spec->id]) }}">
                                                    @csrf
                                                    @method('DELETE')
