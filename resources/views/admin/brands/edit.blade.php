...
<form class="form-template-v3" method="POST" action="{{ route('brands.update', ['brand' => $brand->id]) }}">
    ...
    <fieldset class="margin-bottom-md padding-bottom-md border-bottom">
      <div class="text-component margin-bottom-md text-center">
        <h2>Изменить бренд {{ $brand->name }}</h2>
      ...
      <div class="margin-bottom-xs">
        <div class="grid gap-xxs items-center@md">
          ...
          <div class="col-8@md">
            <input class="form-control width-100%" type="text" name="title" id="title" value="{{ $brand->title }}">
          </div>
        </div>
      </div>
      ...
      <div>
        <div class="grid gap-xxs items-center@md">
          ...
          <div class="col-8@md">
            <textarea class="form-control width-100%" name="about" id="code">{{ $brand->about }}</textarea>
          </div>
        </div>
      </div>
    </fieldset>
