...
<form class="form-template-v3" method="POST" action="{{ route('menuitems.store') }}" enctype="multipart/form-data">
    @csrf
    ...
    <fieldset class="margin-bottom-md padding-bottom-md border-bottom">
      ...
      <div class="margin-bottom-xs">
        <div class="grid gap-xxs items-center@md">
          ...
          <div class="col-8@md">
            <input class="form-control width-100% @error('link') form-control--error @enderror" type="text"  name="link" id="link" value="{{ old('link') }}">
              @error('link')
              <div role="alert" class="bg-error bg-opacity-20% padding-xxxs radius-md text-xs color-contrast-higher margin-top-xxs"><p><strong>Ошибка:</strong> {{ $message }}</p></div>
              @enderror
          </div>
      ...
      <div class="margin-bottom-xs">
        <div class="grid gap-xxs items-center@md">
          ...
          <div class="col-8@md">
            <input class="form-control width-100%" type="number" name="position" id="position" value="{{ old('position') }}">
          </div>
        </div>
      </div>
            ...
            <div class="select">
                <select
                    ...
                    @foreach ($groupmenus as $k => $v)
                        <option value="{{ $k }}" @if($k == old('group_menu_id')) selected @endif>{{ $v }}</option>
                    @endforeach
                </select>
