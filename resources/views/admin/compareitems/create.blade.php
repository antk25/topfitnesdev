...
<form class="form-template-v3" method="POST" action="{{ route('compareitems.store') }}" enctype="multipart/form-data">
    @csrf
    <fieldset class="margin-bottom-md padding-bottom-md border-bottom">
      <div class="text-component margin-bottom-md text-center">
        <h2>Создать compareitem для {{ $comparisons->name }}</h2>
      </div>

      <div class="margin-bottom-xs">
        <label class="form-label margin-y-xs" for="bracelet_id">Браслет-победитель</label>
        <div class="select">
      ...
      <div class="margin-bottom-xs">
        <label class="form-label margin-y-xs" for="comparison_id">Сравнение</label>
       {{ $comparisons->name }} 
      </div>
      ...
      <div class="grid gap-xxs margin-bottom-xs">
        <div class="col-6@md">
          ...
          <p class="text-xs color-contrast-medium margin-top-xxs">Заголовок пункта сравнения</p>
        </div>
      </div>
  
      <div class="margin-bottom-xs">
        <label class="form-label margin-bottom-xxs" for="content">Текст сравнения</label>
    ...
    <section class="bg bg-white padding-sm">
      <p class="color-contrast-medium margin-bottom-sm">Добавить картинки</p>
            ...
            <div class="margin-y-md">
              <button type="submit" class="btn btn--success">Добавить compareitem</button>
            </div>
  </form>
@endsection
