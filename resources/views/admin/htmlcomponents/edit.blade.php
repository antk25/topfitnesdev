...
<form class="form-template-v3" method="POST" action="{{ route('htmlcomponents.update', ['htmlcomponent' => $htmlcomponent->id]) }}">
    @csrf
    @method('PUT')
    ...
    <fieldset class="margin-bottom-md padding-bottom-md border-bottom">
      ...
      <div class="grid gap-xxs margin-bottom-xs">
        <div class="col-6@md">
          ...
          <input class="form-control width-100% @error('name') form-control--error @enderror" type="text" name="name" id="name" value="{{ $htmlcomponent->name }}">
            @error('name')
        ...
        <div class="col-6@md">
          ...
          <input class="form-control width-100% @error('link') form-control--error @enderror" type="url" name="link" id="link" value="{{ $htmlcomponent->link }}">
            @error('link')
            <div role="alert" class="bg-error bg-opacity-20% padding-xxxs radius-md text-xs color-contrast-higher margin-top-xxs"><p><strong>ошибка:</strong> {{ $message }}</p></div>
            @enderror
